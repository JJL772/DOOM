...
void AM_drawMline(mline_t *ml, int color)
{
	...
	if (AM_clipMline(ml, &fl))
		AM_drawFline(&fl,
			     color); // draws it on frame buffer using fb coords
}
	...
	for (i = 0; i < numlines; i++)
	{
		...
		if (cheating || (lines[i].flags & ML_MAPPED))
		{
			...
			else
			{
				...
				{
					AM_drawMline(&l, CDWALLCOLORS + lightlev); // ceiling level
										   // change
				}
				else if (cheating)
				{
...
void AM_drawThings(int colors, int colorrange)
{
	...
	for (i = 0; i < numsectors; i++)
	{
		...
		while (t)
		{
			AM_drawLineCharacter(thintriangle_guy, NUMTHINTRIANGLEGUYLINES, 16 << FRACBITS, t->angle,
...
void AM_drawMarks(void)
{
	...
	for (i = 0; i < AM_NUMMARKPOINTS; i++)
	{
		if (markpoints[i].x != -1)
		{
			...
			w = 5; // because something's wrong with the wad, i
			       // guess
			h = 6; // because something's wrong with the wad, i
			       // guess
