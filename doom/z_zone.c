...
void *Z_Malloc(int size, int tag, void *user)
{
	...
	do
	{
		...
		if (rover->user)
		{
			...
			{
				// free the rover block (adding the size to
				// base)
	...
	else
	{
		if (tag >= PU_PURGELEVEL)
			I_Error("Z_Malloc: an owner is required for purgable "
				"blocks");
...
void Z_DumpHeap(int lowtag, int hightag)
{
	...
	for (block = mainzone->blocklist.next;; block = block->next)
	{
		...
		if ((byte *)block + block->size != (byte *)block->next)
			printf("ERROR: block size does not touch the next "
			       "block\n");

		if (block->next->prev != block)
			printf("ERROR: next block doesn't have proper back "
			       "link\n");
...
void Z_FileDumpHeap(FILE *f)
{
	...
	for (block = mainzone->blocklist.next;; block = block->next)
	{
		...
		if ((byte *)block + block->size != (byte *)block->next)
			fprintf(f, "ERROR: block size does not touch the next "
				   "block\n");

		if (block->next->prev != block)
			fprintf(f, "ERROR: next block doesn't have proper back "
				   "link\n");
...
void Z_CheckHeap(void)
{
	...
	for (block = mainzone->blocklist.next;; block = block->next)
	{
		...
		if ((byte *)block + block->size != (byte *)block->next)
			I_Error("Z_CheckHeap: block size does not touch the "
				"next block\n");

		if (block->next->prev != block)
			I_Error("Z_CheckHeap: next block doesn't have proper "
				"back link\n");
